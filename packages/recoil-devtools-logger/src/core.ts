 ...
 */
const getLogLevel = (level: LogLevel, action: object, newValue: any[], type: string): string => {
  switch (typeof level) {
    case 'object':
      const typeLevel = level[type];
      return typeof typeLevel === 'function' ? typeLevel(...newValue) : typeLevel;
    case 'function':
      return level(action);
    default:
      return level;
...
const defaultTitleFormatter = (options: { timestamp?: any; duration?: any; }) => {
  const { timestamp, duration } = options;

  return (action: { name: string; }, time: any, took: number) => {
    const parts = ['action'];

    parts.push(`%c${String(action.name)}`);
...
const printBuffer = (buffer: any[], options: Options) => {
  ...
  buffer.forEach((logEntry: LogEntry, key: number) => {
    const { started = 0, startedTime = new Date(), action, prevState, error } = logEntry;
